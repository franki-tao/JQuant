package jquant.math.integrals;

import static jquant.math.MathUtils.squared;

public class GaussLaguerreCosinePolynomial extends GaussLaguerreTrigonometricBase {
    private double m0_;

    public GaussLaguerreCosinePolynomial(double u) {
        super(u);
        m0_ = 1.0 + 1.0 / (1.0 + u * u);
    }

    @Override
    protected double m0() {
        return 1 / (1 + super.u_ * super.u_);
    }

    @Override
    protected double m1() {
        return (1 - super.u_ * super.u_) / squared(1 + super.u_ * super.u_);
    }

    @Override
    public double w(double x) {
        return Math.exp(-x) * (1 + Math.cos(super.u_ * x)) / m0_;
    }

    @Override
    public double moment(int n) {
        return (super.moment_(n) + super.fact(n)) / m0_;
    }
}
